...
def train_bias_remover(ann, X, Y, train_set):
    ...
    sizes = np.array([X.shape[1]] + [40]*2 + [1], dtype=np.int32)
    ann_bias = ANN_DLL.ann_create(sizes.ctypes.data, ctypes.c_int(sizes.shape[0]), ctypes.c_int(1))

    for i in range(1):
        ANN_DLL.ann_fit(ctypes.c_void_p(ann_bias), Xtmp.ctypes.data, Ytmp.ctypes.data, ctypes.c_int(MBS), ctypes.addressof(alpha), ctypes.c_double(24.), ctypes.c_int(500))
